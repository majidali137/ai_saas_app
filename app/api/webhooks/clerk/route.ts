...
export async function POST(req: Request) {
  ...
  if (!WEBHOOK_SECRET) {
    ...
    return new Response("Server Error: Missing WEBHOOK_SECRET", { status: 500 });
  }
  ...
  try {
    const { id, email_addresses, image_url, first_name, last_name, username } = evt.data as UserCreatedEventData;
    ...
    if (evt.type === "user.created") {
      ...
      console.log("Creating user from webhook:", user);
      const newUser = await createUser(user);
    ...
    if (evt.type === "user.updated") {
      ...
      console.log("Updating user from webhook:", user);
      const updatedUser = await updateUser(id, user);
    ...
    return new Response("Server Error: Error processing event", { status: 500 });
  }
}
